...
public interface IProcessFilterIpIntervalService {
     ...
     * @return x分钟内访问服务器的ip集合
     */
    List<String> findProIpIntervalRawdataWithoutBlackIpFirst(Timestamp timestamp);


    /**
     * 获取用于【process策略中ipInterval规则分析】的原始数据（第二部分）
     * 查询ip在x分钟内的所有访问时间，以便计算其平均访问间隔
     * @param ip ip地址（该ip不为嫌疑ip）
     * @return 该ip在x分钟内，所有访问时间的集合
     */
    List<Timestamp> findProIpIntervalRawdataWithoutBlackIpSecond(String ip);
     ...
     */
    ProcessIpThresholdList analyzeCrawlersByProIpInterval(ProcessIpThresholdList processIpThresholdList, Timestamp timestamp);
