...
import com.qianyishen.service.IProcessFilterIpUserAgentService;
 ...
 */
@Service("processFilterIpUserAgentService")
public class ProcessFilterIpUserAgentServiceImpl implements IProcessFilterIpUserAgentService {
     ...
     */
    @Override
    public List<ProcessFilterIpUserAgent> findProIpUserAgentRawdataWithoutBlackIp(Timestamp timestamp) {
        //查询已经存在的ip黑名单
        ...
        return crawlerRawDataDao.findAllProIpUserAgentRawdata(crawlerFilteringOperation);
    }

    /**
     * 根据process策略中的ipUserAgent规则分析ip原始数据，从而判断出嫌疑ip
     * 步骤：
     *      1. 先获取【用于process策略中ipUserAgent规则分析】的ip原始数据
     *      2. 获取正在启用的process策略
     ...
     */
    @Override
    public ProcessIpThresholdList analyzeCrawlersByProIpUserAgent(ProcessIpThresholdList processIpThresholdList,Timestamp timestamp) {
        //获取用于process策略的ipUserAgent规则分析的原始ip数据
        List<ProcessFilterIpUserAgent> rawDataList = findProIpUserAgentRawdataWithoutBlackIp(timestamp);
        //获取正在启用的process策略
        ...
        if (rawDataList != null  && rawDataList.size() > 0 && enableProcess != null){
            //获取正在启用的process策略中的ipUserAgent规则（即获取某ip的最大useragent种类数）
            Integer proIpUserAgent = enableProcess.getProIpUserAgent();
            //获取正在启用的process策略中的ipUserAgent规则所设定的阈值
            Integer proIpUserAgentThreshold = enableProcess.getProIpUserAgentThreshold();
            //将从原始数据获取的每个ip的访问次数与ipUserAgent规则依次比较，若违反规则，则将该ip列为嫌疑ip，并将对应的阈值进行累加
            for (ProcessFilterIpUserAgent rawData : rawDataList) {
                //判断是否超出最大ip UserAgent种类数，若超出，将该ip置为嫌疑ip
                if (rawData.getUserAgentCount() > proIpUserAgent){
                    boolean flag = false;
                    //判断ProcessIpThresholdList中的ProcessIpThreshold属性是否为null
                    if (processIpThresholdList.getProcessIpThreshold() == null){
                        //若为空则直接加入该嫌疑ip
                        //使用 bean工厂来获取 ProcessIpThreshold 实体类对象（这样获取的才是多例的）
                        ...
                        processIpThreshold.setSuspectIpThreshold(proIpUserAgentThreshold);
                        ...
                        for (ProcessIpThreshold ipThreshold : processIpThresholdList.getProcessIpThreshold()) {
                            if (ipThreshold.getSuspectIp().equals(rawData.getIp())){
                                ipThreshold.setSuspectIpThreshold(ipThreshold.getSuspectIpThreshold() + proIpUserAgentThreshold);
                                flag =true;
                            }
                        }
